...
- 文件上传表单和上传反馈必须是两条独立 grid 行，不能让上传按钮视觉块压住反馈文案。
- 上传文件区使用固定六槽 slot board：默认显示 6 个只含 `+ 添加文件` 的空槽，每个槽位填入后显示文件格式、文件名、大小或上传结果。
- 支持格式不在每个空槽内重复展示，避免窄槽位文本溢出和横向滚动。
- 最多允许选择 6 个文件；达到上限后禁用继续添加，并提示 `最多上传 6 个文件，请先移除一个文件。`
- 上传文件槽位区域使用 `minmax(0, 1fr)`，在嵌入高度不足时由槽位区域内部滚动消化空间压力。
- 上传反馈区是短状态条：固定预留 40px、最大 48px，两行以内直接显示，更多失败明细在反馈区内部滚动。
- 上传成功状态示例：`上传完成 · 3 个成功 · 0 个失败 · 31 个片段` 必须出现在上传按钮下方，并与按钮保持明确间距。
